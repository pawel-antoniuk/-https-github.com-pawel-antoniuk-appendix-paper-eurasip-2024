...
for i in range(final_iterations):
    ...
    models[i].dev_hrtfs = [f'hrtf{h}' for h in hrtfs_artificial]
    models[i].test_hrtfs =[f'hrtf{h}' for h in hrtfs_human]
    ...
    assert len([h for h in hrtfs_artificial if h in models[i].dev_hrtfs]) == 0
    assert len([h for h in hrtfs_human if h in models[i].test_hrtfs]) == 0
...
save_progress(models, label=f'final_2_ah_all')
