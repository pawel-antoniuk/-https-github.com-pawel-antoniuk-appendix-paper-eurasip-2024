...
def evaluate(evaluation_name, feature_filter_expression, center):
    ...
    df['Location'] = df.AudioFilenames.apply(
        lambda x: float(x.split('_')[-2].replace('azoffset', '')))

    df_center = df[abs(df.Location) < 10]
    df_off_center = df[(abs(df.Location) > 35) & (abs(df.Location) < 45)]
    df_center_size = min(len(df_center), len(df_off_center))
    df_center = df_center[0:df_center_size]
    df_off_center = df_off_center[0:df_center_size]

    assert len(df_center) == len(df_off_center)
    assert len(df_center.index.intersection(df_off_center.index)) == 0

    if center:
        df = df_center.reset_index()
    else:
        df = df_off_center.reset_index()

    recordings_all = np.unique(df['SongNames'])
    features_filter = feature_filter_expression[0]
    features_num = feature_filter_expression[1]
    dev_size = 128
    test_size = 64
    final_iterations = 7
    ...
    def grid_search(objective,
        ...
        for i in range(it_begin, n_param_product):
            ...
            if verbose:
                ...
                print(
                    f'[Grid Search] result: {pprint.pformat(result)} in {int(minutes)} min {seconds:.2f} s')
    ...
    for i in range(final_iterations):
        ...
        models[i].final_model_params = results.best.params

        print(f'[Final] model params: {models[i].final_model_params}')
        print(f'[Final] fit params: {fit_params}')

        models[i].final_model = train_model(
            models[i].final_model_params, fit_params, models[i].X_dev, models[i].y_dev)
        ...
        models[i].elapsed = time.time() - st

        print(f'[Final] score: {models[i].final_score}')

        minutes, seconds = divmod(models[i].elapsed, 60)
        print(f'[Final] duration: {int(minutes)} min {seconds:.2f} s')

        # save final result
        # save_progress(models[i], label=f'final_feature_location_1_{i}')
    ...
    save_progress(
        models, label=f'final_feature_location_{evaluation_name}_all')


ild_itd_ic = [('^(Features_.*_ILD_Raw_.*)'
              '|(Features_.*_ITD_Raw_.*)'
               '|(Features_.*_IC_Raw_.*)$'), (64 + 64 + 64) * 2]
ild_itd = [('^(Features_.*_ILD_Raw_.*)'
           '|(Features_.*_ITD_Raw_.*)$'), (64 + 64) * 2]
itd_ic = [('^(Features_.*_ITD_Raw_.*)'
          '|(Features_.*_IC_Raw_.*)$'), (64 + 64) * 2]
ild_ic = [('^(Features_.*_ILD_Raw_.*)'
          '|(Features_.*_IC_Raw_.*)$'), (64 + 64) * 2]
ild = [('^(Features_.*_ILD_Raw_.*)$'), (64) * 2]
itd = [('^(Features_.*_ITD_Raw_.*)$'), (64) * 2]
ic = [('^(Features_.*_IC_Raw_.*)$'), (64) * 2]

evaluate('ild_itd_center', ild_itd, True)
evaluate('itd_ic_center', itd_ic, True)
evaluate('ild_ic_center', ild_ic, True)
evaluate('ild_center', ild, True)
evaluate('itd_center', itd, True)
evaluate('ic_center', ic, True)

evaluate('ild_itd_off-center', ild_itd, False)
evaluate('itd_ic_off-center', itd_ic, False)
evaluate('ild_ic_off-center', ild_ic, False)
evaluate('ild_off-center', ild, False)
evaluate('itd_off-center', itd, False)
evaluate('ic_off-center', ic, False)
