...
def save_progress(data, label=None, state_dir='state_ha'):
    global ordinal_num
...
for i in range(final_iterations):
    ...
    models[i].dev_hrtfs = [f'hrtf{h}' for h in hrtfs_human]
    models[i].test_hrtfs =[f'hrtf{h}' for h in hrtfs_artificial]
    ...
    # Check AH HA HRTF intersection
    assert len([h for h in hrtfs_human if h in models[i].dev_hrtfs]) == 0
    assert len([h for h in hrtfs_artificial if h in models[i].test_hrtfs]) == 0
...
save_progress(models, label=f'final_2_ha_all') 
