...
def save_progress(data, label=None, state_dir='state_nhrtf'):
    global ordinal_num
...
nhrtf_models = []
for n_train_hrtfs in [1, 2, 3, 5, 7, 10, 15, 20, 25, 30]:
    models = []
    for i in range(final_iterations):
        ...
        # HRTFs splitting
        hrtfs = np.unique([s.split('_')[1] for s in models[i].df_dev['AudioFilenames']])
        np.random.shuffle(hrtfs)
        models[i].n_train_hrtfs = n_train_hrtfs
        models[i].dev_hrtfs = hrtfs[:n_train_hrtfs]
        models[i].test_hrtfs = hrtfs[n_train_hrtfs:]
        models[i].dev_hrtfs_regex = '^' + '|'.join([f'(?:.*?_{hrtf}_.*)' for hrtf in models[i].dev_hrtfs]) + '$'
        models[i].test_hrtfs_regex = '^' + '|'.join([f'(?:.*?_{hrtf}_.*)' for hrtf in models[i].test_hrtfs]) + '$'

        dev_hrtf_mask = models[i].df_dev.AudioFilenames.str.contains(models[i].dev_hrtfs_regex)
        test_hrtf_mask = models[i].df_test.AudioFilenames.str.contains(models[i].test_hrtfs_regex)
        models[i].df_dev = models[i].df_dev[dev_hrtf_mask]
        models[i].df_test = models[i].df_test[test_hrtf_mask]
        ...
        assert dev_size * n_train_hrtfs * 4 \
            == models[i].X_dev.shape[0] \
            == models[i].y_dev.shape[0]
        assert test_size * (30 - n_train_hrtfs) * 4 \
    ...
    print(f'nhrtf: {n_train_hrtfs}, avg: {avg_score}, std: {std_score}, count: {len(all_scores)}')

    nhrtf_models.append(models)

    save_progress(models, label=f'final_all_nhrtf{n_train_hrtfs}')


# [np.mean([m.final_score for m in nhrtf_model]) for nhrtf_model in nhrtf_models]
# [np.std([m.final_score for m in nhrtf_model]) for nhrtf_model in nhrtf_models]