...
# random noise test
df_width_min = min(df['EnsembleAcutalWidth'])
df_width_max = max(df['EnsembleAcutalWidth'])
df_width_size = df['EnsembleAcutalWidth'].shape[0]
df['EnsembleAcutalWidth'] = df_width_min + (df_width_max - df_width_min) * np.random.rand(df_width_size)
...
def save_progress(data, label=None, state_dir='state_noise'):
    global ordinal_num
