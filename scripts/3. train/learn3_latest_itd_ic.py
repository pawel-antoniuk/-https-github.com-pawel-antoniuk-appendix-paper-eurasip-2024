...
features_filter = ('^(Features_.*_ITD_Raw_.*)'
                  '|(Features_.*_IC_Raw_.*)$')
features_num = (64 + 64) * 2
dev_size = 128
test_size = 64
final_iterations = 3
...
for i in range(final_iterations):
    ...
    # save final result
    save_progress(models[i], label=f'final_5_itd_ic_{i}')
...
save_progress(models, label=f'final_5_itd_ic_all')
