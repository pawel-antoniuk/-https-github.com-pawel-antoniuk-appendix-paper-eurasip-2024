...
df['Location'] = df.AudioFilenames.apply(lambda x: float(x.split('_')[-2].replace('azoffset', '')))
...
def grid_search(objective,
    ...
    for i in range(it_begin, n_param_product):
        ...
        if verbose:
            ...
            print(f'[Grid Search] result: {pprint.pformat(result)} in {int(minutes)} min {seconds:.2f} s')
...
for i in range(final_iterations):
    ...
    models[i].final_score = metrics.mean_absolute_error(models[i].y_test, models[i].final_pred)
...
save_progress(models, label=f'final_feature_location_1_all')
